...
MD::MD(const string& path, uint _c, float d, float t): particlecount(pow(_c, 3)*4), threads((uint)ceil(particlecount/64.0f)), dens(d), temp(t), tempo(t), pen(1000), pes(std::vector<Vec3>(1000)), per(-2000, 0) {
	me = this;
...
void MD::Update() {
	...
	for (uint a = 0; a < 40; a++) {
		rdf[a].y = (*((int*)&nrdf[a].x))*2/particlecount;
		nrdf[a].x = 0;
	}
	glUnmapBuffer(GL_SHADER_STORAGE_BUFFER);
	glBindBuffer(GL_SHADER_STORAGE_BUFFER, 0);
